...
$host = 'localhost';
$db = 'feedback_system2'; // Database for Office 1
...
$feedbacks = [];
$result = $conn->query("SELECT * FROM feedback2");
...
if (isset($_GET['delete_id'])) {
    $delete_id = (int) $_GET['delete_id']; // Ensure the ID is an integer
    if ($delete_id > 0) {
        $delete_query = "DELETE FROM feedback2 WHERE id = $delete_id";
        ...
        <tbody>
            <?php foreach ($feedbacks as $feedback): ?>
                <tr>
                    ...
                    <td><a href="?delete_id=<?php echo $feedback['Id']; ?>" class="delete-button" onclick="return confirm('Are you sure you want to delete this feedback?')">Delete</a></td> <!-- Delete button -->
                </tr>
            <?php endforeach; ?>
        </tbody>
    </table>
    ...
    function filterRatings() {
        ...
        if (filteredRatings.length > 0) {
            filteredRatings.forEach(rating => {
                ...
                newRow.innerHTML = `<td>${rating.rating}</td><td>${rating.feedback_text}</td><td>${new Date(rating.created_at).toLocaleDateString()}</td><td><a href="?delete_id=${rating.id}" class="delete-button" onclick="return confirm('Are you sure you want to delete this feedback?')">Delete</a></td>`; <!-- Delete button -->
            });

            // Calculate average rating for filtered data
            const sumFilteredRatings = filteredRatings.reduce((sum, rating) => sum + rating.rating, 0);
            const averageFilteredRating = sumFilteredRatings / filteredRatings.length;
            averageRatingDiv.innerHTML = `Average Rating: ${averageFilteredRating.toFixed(2)}`;
        } else {
            ratingsTable.innerHTML = "<tr><td colspan='4' class='no-data'>No feedbacks found for selected filters.</td></tr>";
            averageRatingDiv.innerHTML = "Average Rating: --";
        }
    }
</script>
